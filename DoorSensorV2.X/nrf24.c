...
#define RF_WAIT_US(x)  __delay_us(x)  /* wait for the given number of micro-seconds */
#define RF_WAIT_MS(x)  __delay_ms(x)  /* wait for the given number of milli-seconds */
#define RF_CE_LOW()    NRF_CE_SetLow()      /* put CE LOW */
#define RF_CE_HIGH()   NRF_CE_SetHigh()       /* put CE HIGH */
#define RF_CSN_LOW()   NRF_CSN_SetLow()      /* put CSN LOW */
#define RF_CSN_HIGH()  NRF_CSN_SetHigh()     /* put CSN HIGH */
...
bool RF_DataIsReady(char *channel)
{
  char status;
  status=RF_ReadRegister(RF24_STATUS);
  status&=RF24_STATUS_RX_P_NO; //getting the status 
  if(status==RF24_STATUS_RX_P_NO_RX_FIFO_NOT_EMPTY)//if is empty
    return false;
  else
    *channel=status>>1;//shift to get actual channel number
     return true;
}


void RF_openWritingPipe(u8 *address)
{
   RF_WriteRegisterData(RF24_TX_ADDR, address, addressSize);
}

void RF_openReadingPipe(u8 number, u8 *address)
{
   u8 baseAddr=RF24_RX_ADDR_P0;
   baseAddr+=number;

   RF_WriteRegisterData(baseAddr, address, addressSize);
